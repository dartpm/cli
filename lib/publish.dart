...
final publishArgParser = ArgParser()..addFlag('help', abbr: 'h', negatable: false);

handlePublish(List<String> arguments) async {
  final ArgResults argResults = publishArgParser.parse(arguments);

  if (argResults['help'] as bool) {
    print('Usage: Publish package to dartpm, run in the project root you want to publish');
    exit(0);
  }
...
//   if (pubspecContent.contains(RegExp(r'^\s*#?\s*publish_to:', multiLine: true))) {
...
String _updatePublishTo(String content) {
  ...
  return content.replaceFirst(RegExp(r'^\s*#?\s*publish_to:.*', multiLine: true), 'publish_to: https://pub.dev');
}
